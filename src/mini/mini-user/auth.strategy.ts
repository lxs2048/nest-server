...
export class AuthStrategy extends PassportStrategy(Strategy) {
  ...
  constructor(
    ...
    super({
      ...
      secretOrKey: config.get('xcx.jwt.secretkey'),
    });
  }
  ...
  async validate(payload: { id: string }) {
    const user = await this.miniUserService.findBy({ id: payload.id });
    // 查询用户信息
    if (!user) customException.fail('用户已删除', 500);
    if (user.status === 0) customException.fail('用户已禁用', 500);
    return user;
  }
}
